...
# ═══════════════════════════════════════════════════════════════
# UTILIDADES DE TEXTO E DETECCAO DE PERIODOS
# ═══════════════════════════════════════════════════════════════

def strip_srt_to_text(srt_content: str) -> str:
    """
    Converte SRT para texto corrido, removendo timestamps e numeros de bloco.
    Se o texto ja for plain text (sem formato SRT), retorna como esta.
    """
    lines = srt_content.strip().split('\n')
    srt_timestamp = re.compile(r'\d{2}:\d{2}:\d{2}[,.]\d{3}\s*-->\s*\d{2}:\d{2}:\d{2}[,.]\d{3}')
    block_number = re.compile(r'^\d+$')

    has_srt = any(srt_timestamp.search(line) for line in lines[:20])
    if not has_srt:
        return srt_content
    ...
    for line in lines:
        ...
        if not line:
            continue
        if block_number.match(line):
            continue
        if srt_timestamp.search(line):
            continue
        text_lines.append(line)

    result = ' '.join(text_lines)
    if len(srt_content) > 0:
        reduction = 100 - len(result) * 100 // len(srt_content)
        print(f"[SRT->TXT] Convertido: {len(srt_content)} chars SRT -> {len(result)} chars texto ({reduction}% menor)")
    return result


# Padroes de narracao brasileira para deteccao de periodos
_HALFTIME_END_PATTERNS = [
    re.compile(r'fim\s+do\s+primeiro\s+tempo', re.IGNORECASE),
    re.compile(r'final\s+d[ao]\s+primeir[ao]\s+(tempo|etapa)', re.IGNORECASE),
    re.compile(r'encerra\s+o\s+primeiro\s+tempo', re.IGNORECASE),
    re.compile(r'apito\s+final\s+do\s+primeiro\s+tempo', re.IGNORECASE),
    re.compile(r'vai\s+pr[ao]\s+intervalo', re.IGNORECASE),
...
_SECOND_HALF_START_PATTERNS = [
    re.compile(r'come[cç]a\s+o\s+segundo\s+tempo', re.IGNORECASE),
    re.compile(r'bola\s+rolando\s+para\s+(a\s+etapa\s+final|o\s+segundo\s+tempo)', re.IGNORECASE),
    re.compile(r'recome[cç]a\s+a\s+partida', re.IGNORECASE),
...
_EXTRA_TIME_PATTERNS = [
    re.compile(r'\bprorroga[cç][aã]o\b', re.IGNORECASE),
    re.compile(r'\btempo\s+extra\b', re.IGNORECASE),
...
def detect_match_periods_from_transcription(
    ...
    if not transcription_text or len(transcription_text) < 200:
        return result
    ...
    for pattern in _HALFTIME_END_PATTERNS:
        for m in pattern.finditer(search_region):
            ...
            rel_pos = abs_pos / text_len
            snippet = transcription_text[max(0, abs_pos - 30):abs_pos + 50]
            halftime_matches.append({
    ...
    if halftime_matches:
        best = min(halftime_matches, key=lambda m: abs(m['position'] - 0.5))
        result['halftime_detected'] = True
        result['halftime_index'] = best['index']
    ...
    if result['halftime_detected']:
        ...
        after_ht = transcription_text[after_ht_start:after_ht_end]
        for pattern in _SECOND_HALF_START_PATTERNS:
            m = pattern.search(after_ht)
            if m:
                abs_pos = after_ht_start + m.start()
                result['second_half_start_index'] = abs_pos
                snippet = transcription_text[max(0, abs_pos - 20):abs_pos + 40]
    ...
    for pattern in _EXTRA_TIME_PATTERNS:
        m = pattern.search(extra_region)
        if m:
            abs_pos = extra_search_start + m.start()
            result['extra_time_detected'] = True
            result['extra_time_index'] = abs_pos

            if is_srt:
                ts_pat = re.compile(r'(\d{2}:\d{2}:\d{2}[,.]\d{3})\s*-->')
                before_text = transcription_text[max(0, abs_pos - 500):abs_pos]
                ts_matches = list(ts_pat.finditer(before_text))
                if ts_matches:
                    result['extra_time_timestamp_seconds'] = _parse_srt_timestamp_to_seconds(ts_matches[-1].group(1))
...
def split_transcription_by_periods(
    transcription_text: str,
    detection_result: dict
) -> dict:
    """
    Divide a transcricao em periodos baseado nos marcadores detectados.

    Returns:
        dict com first_half, second_half, extra_time, split_successful, split_method
    """
    result = {
        'first_half': '',
        'second_half': '',
    ...
    if detection_result.get('halftime_detected'):
        ht_idx = detection_result['halftime_index']
        split_point = detection_result.get('second_half_start_index', -1)
        if split_point < 0:
            split_point = ht_idx

        result['first_half'] = transcription_text[:ht_idx].strip()
        result['second_half'] = transcription_text[split_point:].strip()
        result['split_successful'] = True
        result['split_method'] = 'narration_marker'

        if detection_result.get('extra_time_detected') and detection_result['extra_time_index'] > split_point:
            et_idx = detection_result['extra_time_index']
            result['extra_time'] = transcription_text[et_idx:].strip()
            result['second_half'] = transcription_text[split_point:et_idx].strip()

        print(f"[PERIOD-SPLIT] Marcador: 1T={len(result['first_half'])} chars, "
              f"2T={len(result['second_half'])} chars"
              + (f", ET={len(result['extra_time'])} chars" if result['extra_time'] else ""))
    else:
        mid = text_len // 2
        # Buscar quebra natural perto do meio
        best_break = mid
        for offset in range(0, min(2000, text_len // 10), 10):
            for pos in [mid + offset, mid - offset]:
                if 0 <= pos < text_len - 1:
                    if transcription_text[pos] == '\n':
                        best_break = pos
                        break
            else:
                continue
        ...
        result['first_half'] = transcription_text[:best_break].strip()
        result['second_half'] = transcription_text[best_break:].strip()
        result['split_successful'] = True
        result['split_method'] = 'midpoint_fallback'

        print(f"[PERIOD-SPLIT] Fallback: 1T={len(result['first_half'])} chars, "
              f"2T={len(result['second_half'])} chars")
...
# ═══════════════════════════════════════════════════════════════════════════════
# FUNÇÕES PARA CENTRALIZAÇÃO DE CLIP E DETECÇÃO DE PICO
# ═══════════════════════════════════════════════════════════════════════════════

def block_center_seconds(block: Tuple) -> float:
    """
    Calcula o centro temporal real do bloco SRT (média entre start e end).

    Args:
        block: Tupla SRT (index, hours, minutes, seconds, ms, text)

    Returns:
        Centro temporal em segundos
    """
    if len(block) < 5:
        return 0.0
    _, hours, minutes, seconds, ms, *_ = block
    start_sec = hours * 3600 + minutes * 60 + seconds + (ms / 1000.0)
    # Estimar duração média de um bloco SRT (~2-4 segundos)
    estimated_duration = 3.0
    return start_sec + (estimated_duration / 2)


def find_peak_intensity_block(window_blocks: List[Tuple]) -> Tuple:
    """
    Encontra o bloco com maior intensidade na janela.
    Usa a combinação de hits de 'gol' + score de intensidade.

    Args:
        window_blocks: Lista de blocos SRT

    Returns:
        Bloco com maior intensidade
    """
    if not window_blocks:
        return None

    best_block = window_blocks[len(window_blocks) // 2]  # fallback: centro
    best_score = 0

    for block in window_blocks:
        text = block[5] if len(block) > 5 else ''
        score = count_goal_hits(text) * 2 + intensity_score(text)
        if score > best_score:
            best_score = score
            best_block = block
...
def weighted_anchor_time(window_blocks: List[Tuple]) -> float:
    """
    Calcula tempo âncora ponderado pela intensidade de cada bloco.
    Blocos com mais repetições de 'gol' têm mais peso no cálculo.

    Args:
        window_blocks: Lista de blocos SRT

    Returns:
        Tempo âncora ponderado em segundos
    """
    if not window_blocks:
        return 0.0
    ...
    for block in window_blocks:
        text = block[5] if len(block) > 5 else ''
        # Peso baseado em hits de gol + intensidade + 1 (evita zero)
        weight = count_goal_hits(text) * 2 + intensity_score(text) + 1
        center = block_center_seconds(block)
        weighted_sum += center * weight
        total_weight += weight

    if total_weight > 0:
        return weighted_sum / total_weight
    return block_center_seconds(window_blocks[len(window_blocks) // 2])


def calculate_centered_clip_bounds(
    anchor_seconds: float,
    radius_seconds: float = 10.0,
    compensate_narrator_delay: bool = True,
    narrator_delay: float = 1.5
) -> Dict[str, float]:
    """
    Calcula os limites do clip centralizado no evento.

    Args:
        anchor_seconds: Tempo âncora em segundos
        radius_seconds: Raio simétrico do clip
        compensate_narrator_delay: Se True, compensa delay do narrador
        narrator_delay: Delay do narrador em segundos (padrão 1.5s)

    Returns:
        Dict com clip_start_second, clip_end_second e anchor ajustado
    """
    adjusted_anchor = anchor_seconds

    # Compensar delay do narrador (evento acontece ANTES da reação)
    if compensate_narrator_delay:
        adjusted_anchor = max(0, anchor_seconds - narrator_delay)

    return {
        'clip_start_second': max(0, adjusted_anchor - radius_seconds),
        'clip_end_second': adjusted_anchor + radius_seconds,
        'adjusted_anchor': adjusted_anchor,
        'original_anchor': anchor_seconds,
        'narrator_delay_compensated': compensate_narrator_delay
    }
...
# ═══════════════════════════════════════════════════════════════════════════════
# CONSTANTES E FUNÇÕES PARA CENTRALIZAÇÃO DE CLIP
# ═══════════════════════════════════════════════════════════════════════════════

# Constante para compensação do delay do narrador
# O narrador reage ~1.5 segundos DEPOIS do evento acontecer
NARRATOR_DELAY_SECONDS = 1.5


def ensure_clip_window(
    event: Dict[str, Any],
    back_seconds: int = 20,
    ...
    """
    Gera janela de clip para o evento com suporte a centralização.

    Args:
        event: Dicionário do evento com videoSecond, minute, second
        back_seconds: Segundos antes do evento (ignorado se radius_seconds)
        forward_seconds: Segundos depois do evento (ignorado se radius_seconds)
        radius_seconds: Raio simétrico do clip (centraliza o evento)
        compensate_narrator_delay: Se True, compensa o delay do narrador (~1.5s)
        center_bias: Ajuste manual para antecipar (-) ou atrasar (+) o centro

    Returns:
        Evento atualizado com clip_start_second e clip_end_second
    """
    ...
    # Compensar delay do narrador (evento acontece ANTES da reação)
    if compensate_narrator_delay:
        vs = max(0, vs - NARRATOR_DELAY_SECONDS)

    # Aplicar bias de ajuste manual
    vs = max(0, vs + center_bias)

    event["videoSecond"] = vs
    event["narrator_delay_compensated"] = compensate_narrator_delay

    if radius_seconds is not None:
        # Clip simétrico e centralizado
        event["clip_start_second"] = max(0, vs - int(radius_seconds))
        event["clip_end_second"] = vs + int(radius_seconds)
        event["clip_centered"] = True
        event["clip_radius"] = radius_seconds
    else:
        # Método original (assimétrico)
        event["clip_start_second"] = max(0, vs - int(back_seconds))
        event["clip_end_second"] = vs + int(forward_seconds)
        event["clip_centered"] = False

    return event


def ensure_clip_window_centered(
    event: Dict[str, Any],
    radius_seconds: int = 15,
    compensate_narrator_delay: bool = True
) -> Dict[str, Any]:
    """
    Versão simplificada que sempre centraliza o clip.

    Args:
        event: Dicionário do evento
        radius_seconds: Raio simétrico (segundos antes E depois)
        compensate_narrator_delay: Se True, compensa delay do narrador

    Returns:
        Evento com clip centralizado
    """
    return ensure_clip_window(
        event,
        radius_seconds=radius_seconds,
        compensate_narrator_delay=compensate_narrator_delay
    )
...
def detect_events_by_keywords_from_text(
    ...
    patterns = {
        # GOLS
        'goal': [
            r'go+l', r'golaço', r'bola na rede', r'abre o placar', r'empata',
            r'vira o jogo', r'desempata', r'marca o gol', r'balan(c|ç)ou as redes'
        ],

        # CARTÕES
        'yellow_card': [
            r'cart[aã]o amarelo', r'amarelou', r'leva o amarelo',
            r'punido com amarelo', r'recebe amarelo'
        ],
        'red_card': [
            r'cart[aã]o vermelho', r'expulso', r'expuls(o|ã)o',
            r'red card', r'vai embora mais cedo', r'foi pra rua'
        ],

        # PÊNALTI
        'penalty': [
            r'p[eê]nalti', r'penalidade', r'marca penal', r'marca a cal'
        ],

        # FALTAS
        'foul': [
            r'falta', r'infra[cç][aã]o', r'jogada faltosa',
            r'pega forte', r'entrada dura', r'parou o lance'
        ],

        # IMPEDIMENTO
        'offside': [
            r'impedimento', r'posição irregular', r'estava impedido',
            r'sobe a bandeira', r'bandeira levantada'
        ],

        # ESCANTEIO
        'corner': [
            r'escanteio', r'bola desviada', r'vai pra cobrança de canto',
            r'bola pela linha de fundo desviada'
        ],

        # LATERAL
        'throw_in': [
            r'lateral', r'arremesso lateral', r'cobra o lateral'
        ],

        # SUBSTITUIÇÃO
        'substitution': [
            r'substitui', r'mexe no time', r'entra', r'sai',
            r'troca no time', r'muda a equipe'
        ],

        # DEFESAS
        'save': [
            r'grande defesa', r'salvou', r'espalmou', r'defende o goleiro',
            r'boa defesa', r'pega firme'
        ],

        # LANCES PERIGOSOS
        'dangerous_play': [
            r'lance perigoso', r'quase gol', r'chegada perigosa',
            r'grande chance', r'oportunidade clara', r'perdeu o gol'
        ],

        # BOLA NA TRAVE
        'hit_post': [
            r'na trave', r'acerta o poste', r'trave', r'bateu no travessão'
        ],

        # TIRO DE META
        'goal_kick': [
            r'tiro de meta', r'bola pela linha de fundo'
        ],

        # VAR
        'var_review': [
            r'vai ao var', r'revis[aã]o de lance', r'checa no var',
            r'analisa no var', r'review do var'
        ],
}
    ...
    # 5. Deduplicar por tipo (gols precisam de janela maior)
    events_by_type = {}
    for e in events:
        et = e.get('event_type', 'unknown')
        events_by_type.setdefault(et, []).append(e)

    text_dedup_thresholds = {'goal': 120, 'penalty': 60, 'red_card': 60, 'yellow_card': 45}
    deduped = []
    for etype, type_events in events_by_type.items():
        threshold = text_dedup_thresholds.get(etype, 30)
        deduped.extend(deduplicate_events(type_events, threshold_seconds=threshold))

    events = sorted(deduped, key=lambda e: e.get('videoSecond', 0))
    ...
    if use_ollama_flow:
        try:
            ...
            if events:
                ...
                # ═══════════════════════════════════════════════════════════
                # ENRIQUECIMENTO DE TIMESTAMPS VIA TXT (FONTE PRIMÁRIA)
                # Se eventos têm minute=0, tentar obter timestamps do TXT primeiro
                # ═══════════════════════════════════════════════════════════
                if match_id:
                    try:
                        from storage import get_subfolder_path
                        ...
                        if events_needing_timestamps:
                            ...
                            print(f"[Kakttus] 📄 Buscando timestamps no TXT (fonte primária)...")
                            keyword_events = detect_events_by_keywords_from_text(
                                transcription=transcription,
                                home_team=home_team,
                                away_team=away_team,
                            ...
                            if remaining_events and target_srt:
                                print(f"[Kakttus] ⚠ {len(remaining_events)} eventos ainda sem timestamp, tentando SRT como fallback...")
                                print(f"[Kakttus] 📄 Enriquecendo via SRT: {target_srt.name}")
                                
                                srt_keyword_events = detect_events_by_keywords(
                                    srt_path=str(target_srt),
                                    home_team=home_team,
                                    away_team=away_team,
                                    half=match_half,
