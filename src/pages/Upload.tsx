...
export default function VideoUpload() {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      // Force async pipeline for direct video links (non-YouTube) - server handles download
      ...
      if (!hasTranscription) {
        ...
        setProcessingMessage('Partida criada — transcrição será feita depois');
        
        toast({
          title: "✓ Partida criada com sucesso",
          description: "A transcrição não foi possível agora. Você pode importar um SRT ou reanalisar na página de Eventos.",
          duration: 8000
        });
        
        // Redirect to events page after a moment
        setTimeout(() => {
          navigate(`/events?match=${matchId}`);
        }, 2500);
        return;
                  ...
                  try {
                    ...
                    try {
                      await asyncProcessing.startProcessing({
                        ...
                        autoClip: true,
                        autoAnalysis: true,
                        // Smart Import transcription is only 5min partial - never pass as full transcription
